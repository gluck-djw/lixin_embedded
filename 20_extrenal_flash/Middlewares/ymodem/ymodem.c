          ...
          else
          {
            if (packets_received == 0)
            {
              ...
              if (packet_data[PACKET_HEADER] != 0)
              {
                ...
                // {
                //   FLASHStatus = FLASH_EraseSector(GetSector(FlashDestination + (PageSize * EraseCounter)), VoltageRange_3);
                // }
                // 擦除W25Qxx
//                uint32_t earse_addr = FlashDestination;
//                for (; earse_addr < FlashDestination + size; earse_addr += W25QXXXX_SUBSECTOR_SIZE)
//                {
//                  if (W25Qx_OK != W25Qx_Erase_Block(earse_addr))
//                  {
//                    return -1;
//                  }
//                }
            ...
            {
              if (0 != W25QXX_writehandler(packet_data + PACKET_HEADER, FlashDestination, packet_length))
              {
                Send_Byte(CA);
                Send_Byte(CA);
                return -2;
...
uint8_t Ymodem_Transmit(uint8_t *buf, const uint8_t *sendFileName, uint32_t sizeFile)
{
  ...
  {
    file_name[i] = sendFileName[i];
  }
  CRC16_F = 1;

  /* Prepare first block */
  Ymodem_PrepareIntialPacket(&packet_data[0], file_name, &sizeFile);
