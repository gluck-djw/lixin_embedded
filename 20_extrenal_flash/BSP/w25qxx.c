...
static void W25Qx_Reset(void)
{
    ...
    // SPIx_SendData(cmd, 2);
    // SPI1_Writebyte(&hspi1, cmd, 2, W25QXXXX_TIMEOUT_VALUE);
    W25Qx_Disable();
}
...
uint8_t W25Qx_Read(uint8_t *pData, uint32_t ReadAddr, uint32_t Size)
{
    ...
    SPI1_Writebyte(cmd, 4, W25QXXXX_TIMEOUT_VALUE);
    /* Reception of the data */
    if (SPI1_readbyte(pData, Size, W25QXXXX_TIMEOUT_VALUE) != SPI_OK)
    {
        return W25Qx_ERROR;
    }
    W25Qx_Disable();
...
uint8_t W25Qx_Write(uint8_t *pData, uint32_t WriteAddr, uint32_t Size)
{
    ...
    do
    {
        ...
        W25Qx_Enable();
        /* Send the command */
        if (SPI1_Writebyte(cmd, 4, W25QXXXX_TIMEOUT_VALUE) != SPI_OK)
        {
            return W25Qx_ERROR;
        }

        /* Transmission of the data */
        if (SPI1_Writebyte(pData, current_size, W25QXXXX_TIMEOUT_VALUE) != SPI_OK)
        {
            return W25Qx_ERROR;
        }
        W25Qx_Disable();
