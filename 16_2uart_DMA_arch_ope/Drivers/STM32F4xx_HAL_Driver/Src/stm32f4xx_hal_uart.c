  ...
  if ((huart->ReceptionType == HAL_UART_RECEPTION_TOIDLE) && ((isrflags & USART_SR_IDLE) != 0U) && ((cr1its & USART_SR_IDLE) != 0U))
  {
    ...
    if (HAL_IS_BIT_SET(huart->Instance->CR3, USART_CR3_DMAR))
    {
      ...
      if ((nb_remaining_rx_data > 0U) && (nb_remaining_rx_data < huart->RxXferSize))
      {
        ...
        uart_idle_irq_callback(huart, (huart->RxXferSize - huart->RxXferCount));
#endif /* USE_HAL_UART_REGISTER_CALLBACKS */
      }
      return;
    }
...
static void UART_DMAReceiveCplt(DMA_HandleTypeDef *hdma)
{
  ...
  if (huart->ReceptionType == HAL_UART_RECEPTION_TOIDLE)
  {
    ...
    dma_rx_complete_irq_callback(huart, huart->RxXferSize);
#endif /* USE_HAL_UART_REGISTER_CALLBACKS */
  }
  else
  {
...
static void UART_DMARxHalfCplt(DMA_HandleTypeDef *hdma)
{
  ...
  if (huart->ReceptionType == HAL_UART_RECEPTION_TOIDLE)
  {
    ...
    dma_rx_half_irq_callback(huart, huart->RxXferSize / 2U);
#endif /* USE_HAL_UART_REGISTER_CALLBACKS */
  }
  else
  {
