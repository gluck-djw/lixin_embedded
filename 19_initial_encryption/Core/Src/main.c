...
{
  char str[2 * len + 1]; // ??? 2 ??? + ?? \0

  for (uint8_t i = 0; i < len; i++)
  {
    sprintf(&str[2 * i], "%02x", buf[i]); // ???? str[0..1], str[2..3]...
  }
...
void aes_test(void)
{
  unsigned char AES_128key[16] = "123456789abcdefa";
  unsigned char AES_IV[16] = "0102030405123456";
  uint8_t buf[16] = "Eternal-Chip";
  uint8_t savebuf[16], descryptbuf[16];
  AES_key_expansion(AES_128key);
  AES_encrypt(buf, savebuf, sizeof(buf), AES_IV);
  printf_hex(savebuf, sizeof(savebuf));
