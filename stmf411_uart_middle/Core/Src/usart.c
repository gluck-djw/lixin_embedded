...
static volatile bool if_tx_cpl = 0;
...
/* return 1 when tx complete, others when transmiting */
// static bool if_tx_complete(void)
// {
//     return if_tx_cpl;
// }

/* blocking tx, can be optimized to nonblocking easily */
void t_printf(const char *fmt, ...)
{
    ...
    uint16_t tx_len = (len < (int)sizeof(buffer)) ? (uint16_t)len : (uint16_t)(sizeof(buffer) - 1U);
    if_tx_cpl = 0;
    // HAL_UART_Transmit_DMA(&huart1, (uint8_t *)buffer, tx_len);
    // /* blocking until tx complete */
    // while(!if_tx_complete());
    
    // HAL_UART_Transmit(&huart1, (uint8_t *)buffer, tx_len, 1000);
    HAL_UART_Transmit(&huart2, (uint8_t *)buffer, tx_len, 1000);
}

static uint8_t recv_buffer[RECV_BUF_SIZE];
recv_buf_t g_recv_buf = 
{
...
{
    // UP_DEBUG_GREEN("full_cpl\r\n");
    notify_isr_cb(&g_uart_proto);
}

void HAL_UART_RxHalfCpltCallback(UART_HandleTypeDef *huart)
{
    // UP_DEBUG_YELLOW("half_cpl\r\n");
    notify_isr_cb(&g_uart_proto);
}
